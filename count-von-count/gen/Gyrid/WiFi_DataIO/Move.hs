...
module Gyrid.WiFi_DataIO.Move (Move(..)) where
import Prelude ((+), (/), (.))
...
instance Prelude'.Enum Move where
  fromEnum Move_IN = 1
  fromEnum Move_OUT = 2
  toEnum = P'.fromMaybe (Prelude'.error "hprotoc generated code: toEnum failure for type Gyrid.WiFi_DataIO.Move") . toMaybe'Enum
  succ Move_IN = Move_OUT
  succ _ = Prelude'.error "hprotoc generated code: succ failure for type Gyrid.WiFi_DataIO.Move"
  pred Move_OUT = Move_IN
  pred _ = Prelude'.error "hprotoc generated code: pred failure for type Gyrid.WiFi_DataIO.Move"
...
instance P'.ReflectEnum Move where
  ...
  reflectEnumInfo _
   = P'.EnumInfo (P'.makePNF (P'.pack ".Gyrid.WiFi_DataIO.Move") [] ["Gyrid", "WiFi_DataIO"] "Move")
      ["Gyrid", "WiFi_DataIO", "Move.hs"]
      [(1, "Move_IN"), (2, "Move_OUT")]

instance P'.TextType Move where
  tellT = P'.tellShow
  getT = P'.getRead