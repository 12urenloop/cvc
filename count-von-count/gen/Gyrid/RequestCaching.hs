...
instance P'.TextType RequestCaching where
  tellT = P'.tellSubMessage
  getT = P'.getSubMessage

instance P'.TextMsg RequestCaching where
  textPut msg
   = do
       P'.tellT "enableCaching" (enableCaching msg)
       P'.tellT "clearCache" (clearCache msg)
       P'.tellT "pushCache" (pushCache msg)
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'enableCaching, parse'clearCache, parse'pushCache]) P'.spaces
       Prelude'.return (Prelude'.foldl (\ v f -> f v) P'.defaultValue mods)
    where
        parse'enableCaching
         = P'.try
            (do
               v <- P'.getT "enableCaching"
               Prelude'.return (\ o -> o{enableCaching = v}))
        parse'clearCache
         = P'.try
            (do
               v <- P'.getT "clearCache"
               Prelude'.return (\ o -> o{clearCache = v}))
        parse'pushCache
         = P'.try
            (do
               v <- P'.getT "pushCache"
               Prelude'.return (\ o -> o{pushCache = v}))