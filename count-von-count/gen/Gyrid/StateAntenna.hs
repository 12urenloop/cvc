...
instance P'.TextType StateAntenna where
  tellT = P'.tellSubMessage
  getT = P'.getSubMessage

instance P'.TextMsg StateAntenna where
  textPut msg
   = do
  ...
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'timestamp, parse'sensorMac, parse'angle]) P'.spaces
