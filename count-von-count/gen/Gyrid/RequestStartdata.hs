...
instance P'.TextType RequestStartdata where
  tellT = P'.tellSubMessage
  getT = P'.getSubMessage

instance P'.TextMsg RequestStartdata where
  textPut msg
   = do
       P'.tellT "enableData" (enableData msg)
       P'.tellT "enableBluetoothRaw" (enableBluetoothRaw msg)
       P'.tellT "enableWifiRaw" (enableWifiRaw msg)
       P'.tellT "enableWifiDevRaw" (enableWifiDevRaw msg)
       P'.tellT "enableSensorMac" (enableSensorMac msg)
  textGet
   = do
       mods <- P'.sepEndBy
                (P'.choice
                  [parse'enableData, parse'enableBluetoothRaw, parse'enableWifiRaw, parse'enableWifiDevRaw, parse'enableSensorMac])
                P'.spaces
    ...
    where
        parse'enableData
         = P'.try
            (do
               v <- P'.getT "enableData"
               Prelude'.return (\ o -> o{enableData = v}))
        parse'enableBluetoothRaw
         = P'.try
            (do
               v <- P'.getT "enableBluetoothRaw"
               Prelude'.return (\ o -> o{enableBluetoothRaw = v}))
        parse'enableWifiRaw
         = P'.try
            (do
               v <- P'.getT "enableWifiRaw"
               Prelude'.return (\ o -> o{enableWifiRaw = v}))
        parse'enableWifiDevRaw
         = P'.try
            (do
               v <- P'.getT "enableWifiDevRaw"
               Prelude'.return (\ o -> o{enableWifiDevRaw = v}))
        parse'enableSensorMac
         = P'.try
            (do
               v <- P'.getT "enableSensorMac"
               Prelude'.return (\ o -> o{enableSensorMac = v}))